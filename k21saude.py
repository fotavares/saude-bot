...
bot = telepot.Bot('845427417:AAEe6lm496Xja7t1TnLqPHFTPk8p31KqATI')
if len(sys.argv) > 1:
	data = datetime.now(timezone('Brazil/East')).strftime('%Y%m%d')
	for chat_id in getChats():
		enviada = bot.sendMessage(chat_id[0],"Quantos dias você trabalhou fora do horário de trabalho essa semana?")
		insertQuestion(enviada['message_id'],enviada['text'],data,chat_id[0])
